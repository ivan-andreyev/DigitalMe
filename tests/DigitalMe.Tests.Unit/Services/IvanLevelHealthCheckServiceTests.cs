...
public class IvanLevelHealthCheckServiceTests : IDisposable
{
    ...
    {
        this._mockFileService = new Mock<IFileProcessingService>();
        this._mockWebService = new Mock<IWebNavigationService>();
        this._mockCaptchaService = new Mock<ICaptchaSolvingService>();
        this._mockVoiceService = new Mock<IVoiceService>();
        this._mockIvanService = new Mock<IIvanPersonalityService>();
        this._mockLogger = new Mock<ILogger<IvanLevelHealthCheckService>>();

        this._healthCheckService = new IvanLevelHealthCheckService(
            this._mockFileService.Object,
            this._mockWebService.Object,
            this._mockCaptchaService.Object,
            this._mockVoiceService.Object,
            this._mockIvanService.Object,
    ...
    {
        // Arrange
        this.SetupHealthyServices();

        // Act
        var result = await this._healthCheckService.CheckAllServicesAsync();
    ...
    {
        // Arrange
        this.SetupPartiallyHealthyServices();
    ...
    public async Task CheckServiceHealthAsync_FileProcessingService_WithValidService_ShouldReturnHealthy()
    {
        ...
        this._mockFileService.Setup(x => x.ProcessPdfAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<Dictionary<string, object>>()))
            .ReturnsAsync((string operation, string filePath, Dictionary<string, object> parameters) =>
            {
                ...
                if (!File.Exists(filePath))
                {
                    ...
                    this._tempFilesToCleanup.Add(filePath); // Track for cleanup
                }
                return mockResult;
            });
        this._mockFileService.Setup(x => x.ExtractTextAsync(It.IsAny<string>()))
            .ReturnsAsync("Health check test content");

        // Act
        var result = await this._healthCheckService.CheckServiceHealthAsync("fileprocessing");
    ...
    public async Task CheckServiceHealthAsync_WebNavigationService_WithInitializedBrowser_ShouldReturnHealthy()
    {
        ...
        this._mockWebService.Setup(x => x.InitializeBrowserAsync(It.IsAny<BrowserOptions?>()))
            .ReturnsAsync(initResult);
        this._mockWebService.Setup(x => x.IsBrowserReadyAsync())
            .ReturnsAsync(true);
        this._mockWebService.Setup(x => x.DisposeBrowserAsync())
            .ReturnsAsync(WebNavigationResult.SuccessResult());

        // Act
        var result = await this._healthCheckService.CheckServiceHealthAsync("webnavigation");
    ...
    {
        // Arrange
        this._mockCaptchaService.Setup(x => x.IsServiceAvailableAsync())
            .ReturnsAsync(true);

        // Act
        var result = await this._healthCheckService.CheckServiceHealthAsync("captchasolving");
    ...
    {
        // Arrange
        this._mockVoiceService.Setup(x => x.IsServiceAvailableAsync())
            .ReturnsAsync(true);
        this._mockVoiceService.Setup(x => x.GetAvailableVoicesAsync())
            .ReturnsAsync(VoiceResult.SuccessResult(new[] { "alloy", "echo", "nova" }));
        this._mockVoiceService.Setup(x => x.GetSupportedAudioFormatsAsync())
            .ReturnsAsync(VoiceResult.SuccessResult(new[] { "mp3", "wav" }));

        // Act
        var result = await this._healthCheckService.CheckServiceHealthAsync("voice");
    ...
    public async Task CheckServiceHealthAsync_IvanPersonalityService_WithValidPersonality_ShouldReturnHealthy()
    {
        ...
        this._mockIvanService.Setup(x => x.GetIvanPersonalityAsync())
            .ReturnsAsync(mockPersonality);
        this._mockIvanService.Setup(x => x.GenerateSystemPrompt(It.IsAny<PersonalityProfile>()))
            .Returns("System prompt for Ivan with structured decision making...");
        this._mockIvanService.Setup(x => x.GenerateEnhancedSystemPromptAsync())
            .ReturnsAsync("Enhanced system prompt for Ivan with detailed personality data...");

        // Act
        var result = await this._healthCheckService.CheckServiceHealthAsync("ivanpersonality");
    ...
    {
        // Act
        var result = await this._healthCheckService.CheckServiceHealthAsync("unknownservice");
    ...
    public async Task GetReadinessScoreAsync_WithAllHealthyServices_ShouldReturnFullScore()
    {
        ...
        // Act
        var score = await this._healthCheckService.GetReadinessScoreAsync();
    ...
    public async Task CheckAllServicesAsync_ShouldLogHealthCheckStart()
    {
        ...
        // Act
        await this._healthCheckService.CheckAllServicesAsync();
    ...
    public void Dispose()
    {
        ...
        foreach (var tempFile in this._tempFilesToCleanup)
        {
            try
            {
    ...
    private void SetupHealthyServices()
    {
        ...
        this._mockFileService.Setup(x => x.ExtractTextAsync(It.IsAny<string>())).ReturnsAsync("Health check test content");

        // Web Navigation
        var webResult = new WebNavigationResult { Success = true, Message = "Browser initialized" };
        this._mockWebService.Setup(x => x.InitializeBrowserAsync(It.IsAny<BrowserOptions?>())).ReturnsAsync(webResult);
        this._mockWebService.Setup(x => x.IsBrowserReadyAsync()).ReturnsAsync(true);
        this._mockWebService.Setup(x => x.DisposeBrowserAsync()).ReturnsAsync(WebNavigationResult.SuccessResult());

        // CAPTCHA Solving
        this._mockCaptchaService.Setup(x => x.IsServiceAvailableAsync()).ReturnsAsync(true);

        // Voice Service
        this._mockVoiceService.Setup(x => x.IsServiceAvailableAsync()).ReturnsAsync(true);
        this._mockVoiceService.Setup(x => x.GetAvailableVoicesAsync()).ReturnsAsync(VoiceResult.SuccessResult(new[] { "alloy", "echo" }));
        this._mockVoiceService.Setup(x => x.GetSupportedAudioFormatsAsync()).ReturnsAsync(VoiceResult.SuccessResult(new[] { "mp3", "wav" }));

        // Ivan Personality
        var personality = new PersonalityProfile { Name = "Ivan Digital Clone" };
        this._mockIvanService.Setup(x => x.GetIvanPersonalityAsync()).ReturnsAsync(personality);
        this._mockIvanService.Setup(x => x.GenerateSystemPrompt(It.IsAny<PersonalityProfile>())).Returns("Ivan system prompt");
        this._mockIvanService.Setup(x => x.GenerateEnhancedSystemPromptAsync()).ReturnsAsync("Enhanced Ivan system prompt");
    }
    ...
    private void SetupPartiallyHealthyServices()
    {
        ...
        this._mockWebService.Setup(x => x.InitializeBrowserAsync(It.IsAny<BrowserOptions?>())).Throws(new Exception("Browser failed to start"));
        ...
        this._mockVoiceService.Setup(x => x.IsServiceAvailableAsync()).ReturnsAsync(true);
        this._mockVoiceService.Setup(x => x.GetAvailableVoicesAsync()).ReturnsAsync(VoiceResult.SuccessResult(new[] { "alloy" }));
        this._mockVoiceService.Setup(x => x.GetSupportedAudioFormatsAsync()).ReturnsAsync(VoiceResult.SuccessResult(new[] { "mp3" }));

        // Ivan Personality - Unhealthy
        this._mockIvanService.Setup(x => x.GetIvanPersonalityAsync()).ThrowsAsync(new Exception("Profile data not found"));
    }
