...
public class MessageProcessor : IMessageProcessor
{
    ...
    public async Task<Result<ProcessMessageResult>> ProcessUserMessageAsync(ChatRequestDto request)
    {
        ...
        }, $"Failed to process user message for UserId: {request.UserId}");
    }

    public async Task<Result<ProcessAgentResponseResult>> ProcessAgentResponseAsync(ChatRequestDto request, Guid conversationId)
    {
        return await ResultExtensions.TryAsync(async () =>
        {
            ...
            {
                _logger.LogError("❌ Ivan's personality profile not found: {Error}", personalityResult.Error);
                throw new InvalidOperationException($"Ivan's personality profile not found: {personalityResult.Error}");
            }
        ...
        }, $"Failed to process agent response for ConversationId: {conversationId}");
    }
}
