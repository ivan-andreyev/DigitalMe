...
public interface ISignalRChatService
{
    ...
    Task JoinChatAsync(string userId, string platform = "Web");
...
public class SignalRChatService : ISignalRChatService
{
    ...
    public SignalRChatService(IOptions<DigitalMeConfiguration> configuration, ILogger<SignalRChatService> logger)
    {
    ...
    public async Task JoinChatAsync(string userId, string platform = "Web")
    {
        if (_hubConnection?.State != HubConnectionState.Connected)
        {
            throw new InvalidOperationException("SignalR connection is not established");
...
// DTO classes are imported from DigitalMe.DTOs
